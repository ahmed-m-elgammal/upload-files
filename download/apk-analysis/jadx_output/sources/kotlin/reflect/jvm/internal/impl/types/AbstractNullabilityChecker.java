        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (none == null) {
                    ...
                    while (it.hasNext()) {
                        SimpleTypeMarker mo3851transformType = none.mo3851transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(mo3851transformType) && !typeSystemContext.isMarkedNullable(mo3851transformType)) || typeSystemContext.isDefinitelyNotNullType(mo3851transformType)) {
                            typeCheckerState.clear();
                            return true;
                        }
                        supertypesDeque.add(mo3851transformType);
                    }
                }
            }
        }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker mo3851transformType = supertypesPolicy.mo3851transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, mo3851transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(mo3851transformType);
