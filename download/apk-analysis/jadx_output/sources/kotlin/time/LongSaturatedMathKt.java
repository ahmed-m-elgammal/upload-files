...
public final class LongSaturatedMathKt {
    ...
    public static final long m4038saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long m3955toLongimpl = Duration.m3955toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m4037checkInfiniteSumDefinedPjuGub4(j, j2, m3955toLongimpl);
        }
        if ((1 | (m3955toLongimpl - 1)) == Long.MAX_VALUE) {
            return m4039saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + m3955toLongimpl;
        return ((j ^ j3) & (m3955toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m4037checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m3941isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m4039saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long m3912divUwyO8pc = Duration.m3912divUwyO8pc(j2, 2);
        long m3955toLongimpl = Duration.m3955toLongimpl(m3912divUwyO8pc, durationUnit);
        return (1 | (m3955toLongimpl - 1)) == Long.MAX_VALUE ? m3955toLongimpl : m4038saturatingAddNuflL3o(m4038saturatingAddNuflL3o(j, durationUnit, m3912divUwyO8pc), durationUnit, Duration.m3944minusLRDsOJo(j2, m3912divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m4011getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m4010getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m3962unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m3945plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m3962unaryMinusUwyO8pc(infinityOfSign(j3));
        }
