...
public final class ImageLibraryContract implements AppContextActivityResultContract<ImageLibraryContractOptions, ImagePickerContractResult> {
    ...
    public ImagePickerContractResult parseResult(ImageLibraryContractOptions input, int resultCode, Intent intent) {
        ...
        ImagePickerContractResult.Error error;
        Intrinsics.checkNotNullParameter(input, "input");
        if (resultCode == 0) {
            ...
            if (intent2 != null && (allDataUris = ImagePickerUtilsKt.getAllDataUris(intent2)) != null) {
                if (input.getOptions().getAllowsMultipleSelection()) {
                    ...
                    if (input.getOptions().getSelectionLimit() > 0) {
                        arrayList2 = CollectionsKt.take(arrayList2, input.getOptions().getSelectionLimit());
                    }
                    error = new ImagePickerContractResult.Success(arrayList2);
                    ...
                    if (uri2 != null) {
                        error = new ImagePickerContractResult.Success(CollectionsKt.listOf(TuplesKt.to(ImagePickerUtilsKt.toMediaType(uri2, getContentResolver()), uri2)));
                    } else {
                        error = ImagePickerContractResult.Error.INSTANCE;
                    }
                }
                if (error != null) {
                    return error;
    ...
    private final Intent createLegacyIntent(ImagePickerOptions options) {
        ...
        int i = WhenMappings.$EnumSwitchMapping$0[options.getMediaTypes().ordinal()];
        if (i == 1) {
            strArr = new String[]{MediaTypes.VideoAllMimeType};
        } else if (i == 2) {
            strArr = new String[]{MediaTypes.ImageAllMimeType};
        } else {
            strArr = new String[]{MediaTypes.ImageAllMimeType, MediaTypes.VideoAllMimeType};
        }
        Intent putExtra = type.putExtra("android.intent.extra.MIME_TYPES", strArr);
