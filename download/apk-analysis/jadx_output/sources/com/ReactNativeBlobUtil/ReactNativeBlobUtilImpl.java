...
class ReactNativeBlobUtilImpl {
    ...
    public void actionViewIntent(String str, String str2, @Nullable String str3, final Promise promise) {
        ...
        try {
            if (!ReactNativeBlobUtilUtils.isContentUri(str)) {
                parse = FileProvider.getUriForFile(RCTContext, RCTContext.getPackageName() + ".provider", new File(str));
            } else {
                parse = Uri.parse(str);
            }
    ...
    public void addCompleteDownload(ReadableMap readableMap, Promise promise) {
        ...
        String normalizePath = ReactNativeBlobUtilUtils.normalizePath(readableMap.getString("path"));
        if (normalizePath == null) {
            promise.reject("EINVAL", "ReactNativeBlobUtil.addCompleteDownload can not resolve URI:" + readableMap.getString("path"));
        } else {
            try {
                downloadManager.addCompletedDownload(readableMap.hasKey("title") ? readableMap.getString("title") : "", readableMap.hasKey("description") ? readableMap.getString("description") : "", true, readableMap.hasKey("mime") ? readableMap.getString("mime") : null, normalizePath, Long.valueOf(ReactNativeBlobUtilFS.statFile(normalizePath).getString(RRWebVideoEvent.JsonKeys.SIZE)).longValue(), readableMap.hasKey("showNotification") && readableMap.getBoolean("showNotification"));
                promise.resolve(null);
            } catch (Exception e) {
                promise.reject("EUNSPECIFIED", e.getLocalizedMessage());
            }
