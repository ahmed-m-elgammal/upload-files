...
public final class BufferFrameLoader implements FrameLoader {
    ...
    public FrameResult getFrame(int frameNumber, int width, int height) {
        ...
        if (bufferFrame != null) {
            ...
            return new FrameResult(bufferFrame.getBitmapRef().mo965clone(), FrameResult.FrameType.SUCCESS);
        }
        loadNextFrames(width, height);
    ...
    private final FrameResult findNearestToRender(int targetFrame) {
        ...
        if (findNearestFrame != null) {
            CloseableReference<Bitmap> mo965clone = findNearestFrame.getBitmap().mo965clone();
            Intrinsics.checkNotNullExpressionValue(mo965clone, "nearestFrame.bitmap.clone()");
            this.lastRenderedFrameNumber = findNearestFrame.getFrameNumber();
            return new FrameResult(mo965clone, FrameResult.FrameType.NEAREST);
        }
