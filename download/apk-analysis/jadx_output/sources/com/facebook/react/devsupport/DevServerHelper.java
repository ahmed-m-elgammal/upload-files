...
public class DevServerHelper {
    ...
    private String createBundleURL(String str, BundleType bundleType, String str2, boolean z, boolean z2) {
        ...
        Object obj = z ? ServerProtocol.DIALOG_RETURN_SCOPES_TRUE : Constants.CASEFIRST_FALSE;
        if (!z2) {
            str4 = Constants.CASEFIRST_FALSE;
        }
        return String.format(locale, "http://%s/%s.%s?platform=android&dev=%s&lazy=%s&minify=%s&app=%s&modulesOnly=%s&runModule=%s", str2, str, typeID, valueOf, valueOf2, valueOf3, str3, obj, str4);
    }
    ...
    public void openDebugger(final ReactContext reactContext, final String str) {
        this.mClient.newCall(new Request.Builder().url(String.format(Locale.US, "http://%s/open-debugger?appId=%s&device=%s", this.mPackagerConnectionSettings.getServerIp(), Uri.encode(this.mPackageName), Uri.encode(getInspectorDeviceId()))).method("POST", RequestBody.create((MediaType) null, "")).build()).enqueue(new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.6
            @Override // okhttp3.Callback
            public void onResponse(Call call, Response response) {
            }
