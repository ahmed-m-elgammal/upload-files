...
class SpringAnimation extends AnimationDriver {
    ...
    public void resetConfig(ReadableMap readableMap) {
        ...
        this.mCurrentLoop = 0;
        this.mTimeAccumulator = AudioStats.AUDIO_AMPLITUDE_NONE;
    ...
    private boolean isAtRest() {
        return Math.abs(this.mCurrentState.velocity) <= this.mRestSpeedThreshold && (getDisplacementDistanceForState(this.mCurrentState) <= this.mDisplacementFromRestThreshold || this.mSpringStiffness == AudioStats.AUDIO_AMPLITUDE_NONE);
    }

    private boolean isOvershooting() {
        return this.mSpringStiffness > AudioStats.AUDIO_AMPLITUDE_NONE && ((this.mStartValue < this.mEndValue && this.mCurrentState.position > this.mEndValue) || (this.mStartValue > this.mEndValue && this.mCurrentState.position < this.mEndValue));
    }
    ...
    private void advance(double d) {
        ...
        if (isAtRest() || (this.mOvershootClampingEnabled && isOvershooting())) {
            if (this.mSpringStiffness > AudioStats.AUDIO_AMPLITUDE_NONE) {
            ...
            this.mCurrentState.velocity = AudioStats.AUDIO_AMPLITUDE_NONE;
        }
    }
}
