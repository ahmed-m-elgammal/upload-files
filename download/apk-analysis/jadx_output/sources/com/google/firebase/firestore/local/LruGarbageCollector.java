...
public class LruGarbageCollector {
        ...
        private void scheduleGC() {
            this.gcTask = this.asyncQueue.enqueueAfterDelay(AsyncQueue.TimerId.GARBAGE_COLLECTION, this.hasRun ? LruGarbageCollector.REGULAR_GC_DELAY_MS : LruGarbageCollector.INITIAL_GC_DELAY_MS, new Runnable() { // from class: com.google.firebase.firestore.local.LruGarbageCollector$GCScheduler$$ExternalSyntheticLambda0
                ...
                public final void run() {
                    LruGarbageCollector.GCScheduler.this.m1646xb32188f8();
                }
            });
        }
    ...
    private Results runGarbageCollection(SparseArray<?> sparseArray) {
        ...
        if (Logger.isDebugEnabled()) {
            ...
            sb.append(str);
            z = true;
            sb.append(String.format(Locale.ROOT, "\tRemoved %d documents in %dms\n", Integer.valueOf(removeOrphanedDocuments), Long.valueOf(currentTimeMillis5 - currentTimeMillis4)));
            Logger.debug("LruGarbageCollector", sb.toString() + String.format(Locale.ROOT, "Total Duration: %dms", Long.valueOf(currentTimeMillis5 - currentTimeMillis)), new Object[0]);
        } else {
            z = true;
        }
        return new Results(z, calculateQueryCount, removeTargets, removeOrphanedDocuments);
    }
