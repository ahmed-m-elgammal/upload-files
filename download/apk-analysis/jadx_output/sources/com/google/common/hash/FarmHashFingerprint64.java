...
final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
    ...
    private static long hashLength0to16(byte[] bytes, int offset, int length) {
        ...
        if (length >= 4) {
            return hashLength16(length + ((LittleEndianByteArray.load32(bytes, offset) & BodyPartID.bodyIdMax) << 3), LittleEndianByteArray.load32(bytes, (offset + length) - 4) & BodyPartID.bodyIdMax, (length * 2) + K2);
        }
        if (length <= 0) {
            return K2;
        }
