...
public final class SimpleFormatter<T> implements TemporalFormatter<T> {
    ...
    private T parseInternal(CharSequence charSequence, ParsePosition parsePosition, RawValues rawValues) {
        ...
        if (this.type.equals(PlainDate.class)) {
            ...
            inZonalView = TemporalType.JAVA_UTIL_DATE.translate(upLocal.parse(charSequence2, parsePosition)).toZonalTimestamp(ZonalOffset.UTC).getCalendarDate();
            updateRawValues(rawValues, upLocal);
        } else if (this.type.equals(PlainTime.class)) {
            SimpleDateFormat upLocal2 = setUpLocal();
            inZonalView = TemporalType.JAVA_UTIL_DATE.translate(upLocal2.parse(charSequence2, parsePosition)).toZonalTimestamp(ZonalOffset.UTC).getWallTime();
            updateRawValues(rawValues, upLocal2);
        } else if (this.type.equals(PlainTimestamp.class)) {
            SimpleDateFormat upLocal3 = setUpLocal();
            inZonalView = TemporalType.JAVA_UTIL_DATE.translate(upLocal3.parse(charSequence2, parsePosition)).toZonalTimestamp(ZonalOffset.UTC);
            ...
            if (this.type.equals(Moment.class)) {
                ...
                if (str.equals(RFC_1123_PATTERN)) {
                    ...
                    if (substring.length() >= 4 && substring.charAt(3) == ',') {
                        str = RFC_1123_WIDE;
                    } else {
                        str = RFC_1123_SHORT;
                    }
        ...
        return this.type.cast(inZonalView);
    }
