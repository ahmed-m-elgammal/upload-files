...
public final class HebrewCalendar extends Calendrical<Unit, HebrewCalendar> implements LocalizedPatternSupport {
    ...
    private static class Transformer implements EraYearMonthDaySystem<HebrewCalendar> {
        ...
        public HebrewCalendar transform(long j) {
            ...
            for (int i3 = 1; i3 < 13; i3++) {
                if (i3 != 6 || isLeapYear) {
                    long lengthOfMonth = hcNewYear - HebrewCalendar.lengthOfMonth(i, HebrewMonth.valueOf(i3));
                    if (lengthOfMonth <= 0) {
                        break;
                    }
                    i2 = i3 + 1;
            ...
            return HebrewCalendar.of(i, HebrewMonth.valueOf(i2), (int) hcNewYear);
        }
    ...
    private static class Merger implements ChronoMerger<HebrewCalendar> {
        ...
        public String getFormatPattern(DisplayStyle displayStyle, Locale locale) {
            return GenericDatePatterns.get(LocalePreferences.CalendarType.HEBREW, displayStyle, locale);
        }
    ...
    private static class HebrewUnitRule implements UnitRule<HebrewCalendar> {
        ...
        public HebrewCalendar addTo(HebrewCalendar hebrewCalendar, long j) {
            int i;
            int i2 = AnonymousClass3.$SwitchMap$net$time4j$calendar$HebrewCalendar$Unit[this.unit.ordinal()];
            if (i2 == 1) {
            ...
            int i3 = hebrewCalendar.year;
            int value = hebrewCalendar.month.getValue();
            for (long abs = Math.abs(j); abs > 0; abs--) {
                if (j > 0) {
                    i = value + 1;
                    if (i != 6 || HebrewCalendar.isLeapYear(i3)) {
                        if (i == 14) {
                            i3++;
                            value = 1;
                        }
            ...
            return HebrewCalendar.of(i3, valueOf, Math.min(hebrewCalendar.dom, HebrewCalendar.lengthOfMonth(i3, valueOf)));
        }
