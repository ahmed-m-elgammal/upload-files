...
public final class Camera2DeviceSurfaceManager implements CameraDeviceSurfaceManager {
    ...
    @Override // androidx.camera.core.impl.CameraDeviceSurfaceManager
    public Pair<Map<UseCaseConfig<?>, StreamSpec>, Map<AttachedSurfaceInfo, StreamSpec>> getSuggestedStreamSpecs(int i, String str, List<AttachedSurfaceInfo> list, Map<UseCaseConfig<?>, List<Size>> map, boolean z) {
        Preconditions.checkArgument(!map.isEmpty(), "No new use cases to be bound.");
        SupportedSurfaceCombination supportedSurfaceCombination = this.mCameraSupportedSurfaceCombinationMap.get(str);
        if (supportedSurfaceCombination == null) {
            throw new IllegalArgumentException("No such camera id in supported combination list: " + str);
        }
        return supportedSurfaceCombination.getSuggestedStreamSpecifications(i, list, map, z);
    }
}
