...
class VirtualCameraAdapter implements UseCase.StateChangeCallback {
    ...
    void bindChildren() {
        for (UseCase useCase : this.mChildren) {
            useCase.bindToCamera((CameraInternal) Objects.requireNonNull(this.mChildrenVirtualCameras.get(useCase)), null, useCase.getDefaultConfig(true, this.mUseCaseConfigFactory));
        }
    }
    ...
    Map<UseCase, SurfaceProcessorNode.OutConfig> getChildrenOutConfigs(SurfaceEdge surfaceEdge, int i, boolean z) {
        ...
        for (UseCase useCase : this.mChildren) {
            Pair<Rect, Size> preferredChildSizePair = this.mResolutionsMerger.getPreferredChildSizePair((UseCaseConfig) Objects.requireNonNull(this.mChildrenConfigsMap.get(useCase)), surfaceEdge.getCropRect(), TransformUtils.getRotationDegrees(surfaceEdge.getSensorToBufferTransform()), z);
            ...
            hashMap.put(useCase, SurfaceProcessorNode.OutConfig.of(getChildTargetType(useCase), getChildFormat(useCase), rect, TransformUtils.rotateSize(size, within360), within360, useCase.isMirroringRequired(this.mParentCamera) ^ isMirrored));
        }
        return hashMap;
    }
